/* Mockup Website/assets/js/page-furniture.js */
// Logika khusus HANYA untuk halaman furniture.html

document.addEventListener("DOMContentLoaded", function () {
    // --- PENGATURAN AWAL ---
    const nomorWA = "6287775080483"; // Nomor WA untuk tombol "Tanya"
    const PRODUK_PER_HALAMAN = 8; // Mengatur paginasi
    ...
    // Validasi: Hentikan script jika elemen penting tidak ditemukan
    ...
    // --- SUMBER DATA (STATE) ---
    let masterProductList = []; // Daftar semua produk dari JSON (HANYA SEKALI LOAD)
    let currentState = {
        kategori: "semua",
        sort: "terbaru",
        halaman: 1
    ...
    /**
     * Fungsi Asinkron untuk memulai halaman:
     * 1. Ambil data JSON
     * 2. Ubah JSON jadi elemen HTML
     * 3. Render
     * 4. Pasang Listeners
     */
    async function initializeFurniturePage() {
        try {
            ...
            // Kita lakukan ini sekali saja untuk performa.
            masterProductList = productsData.map((product) => {
                return {
                    element: createProductCardElement(product), // Elemen HTML (kartu)
                    kategori: product.kategori.toLowerCase(),
                    price: product.harga,
                    id: product.id
                    // (Kita bisa tambah 'date' atau 'popularity' di sini jika JSON-nya punya)
                };
            });
            ...
            // 4. Pasang semua 'listeners' (untuk filter, sort, dll)
            setupListeners();
        } catch (error) {
    ...
    /**
     * Membuat satu kartu HTML (sebagai elemen DOM) dari satu objek produk.
     */
    function createProductCardElement(product) {
        ...
        `
            ...
            .join(""); // .join() untuk menggabungkan array jadi string

        // Buat elemen <div> kolom
        const colDiv = document.createElement("div");
        colDiv.className = `col-lg-3 col-md-4 col-6`; // Responsif
        ...
        return colDiv; // Kembalikan elemen DOM
    }

    /**
     * Memasang semua event listener untuk filter, sortir, dan paginasi.
     * Dijalankan sekali saat inisialisasi.
     */
    function setupListeners() {
        // Listener Filter Kategori (menggunakan event delegation)
        kategoriFilter.addEventListener("click", function (event) {
            ...
            currentState.halaman = 1; // Reset ke halaman 1
            // Render ulang
            renderProducts();
        });
        ...
        // Listener Paginasi (menggunakan event delegation)
        paginationContainer.addEventListener("click", function (event) {
            ...
            renderProducts();
            // Auto-scroll ke atas grid produk
     ...
     * Fungsi ini yang "menggambar ulang" grid produk berdasarkan 'currentState'
     */
    function renderProducts() {
        // 1. FILTER: Ambil dari 'masterProductList'
        ...
        // 2. SORTIR:
        // Buat salinan (...) agar tidak mengubah urutan 'masterProductList'
        ...
        // (Note: 'terbaru' adalah default, jadi tidak perlu di-sort jika masterList sudah urut)
        ...
        // 4. RENDER PRODUK KE DOM
        productContainer.innerHTML = ""; // Kosongkan grid
        if (productsToShow.length === 0) {
            productContainer.innerHTML =
                '<p class="col-12 text-center text-muted py-5">Tidak ada produk yang sesuai dengan filter Anda.</p>';
        } else {
            productsToShow.forEach((product) => {
                productContainer.appendChild(product.element); // Masukkan elemen kartu
            });
        }
    ...
    function renderPagination(totalHalaman, halamanAktif) {
        ...
        if (totalHalaman <= 1) return; // Tidak perlu paginasi
